...
static struct option longopts[] =
  {
    ...
    {"num-det-bursts",    required_argument, (int *) &param.num_det_bursts,	0},
    ...
    {"wlatency-dist",	 required_argument, (int *) &param.wlatency_dist,            0},
    {"olatency-dist",	 required_argument, (int *) &param.olatency_dist,            0},
    {"wlog",	     required_argument, (int *) &param.wlog,            0},
    {"wrangelog",	     required_argument, (int *) &param.wrangelog,            0},
...
usage (void)
{
  printf ("Usage: %s "
	  ...
	  "[--version]\n"
	  "\t[--wlatency-dist y|n,file --olatency-dist out_file] [--wrangelog y|n,file]"
...
{
  extern Load_Generator uri_fixed, uri_wlog, uri_wrangelog, uri_wset, conn_rate, call_seq;
  ...
  while ((ch = getopt_long (argc, argv, "d:hvV", longopts, &longindex)) >= 0)
	...
	case 0:
	  ...
	  else if (flag == &param.num_det_bursts)
	  	    {
	  	      errno = 0;
	  	      param.num_det_bursts = strtoul (optarg, &end, 10);
	  	      if (errno == ERANGE || end == optarg || *end)
	  		{
	  		  fprintf (stderr, "%s: illegal number of det-bursts %s\n",
	  			   prog_name, optarg);
	  		  exit (1);
	  		}
	  	    }
	  ...
	  else if (flag == &param.wlatency_dist)
	  {
		  param.wlatency_dist.show_latency =  (*optarg == 'y') || (*optarg == 'Y');
		  param.wlatency_dist.file = optarg+2;

	  }
	  else if (flag == &param.olatency_dist)
	  {
		  param.olatency_dist.file = optarg;

	  }

	  else if (flag == &param.wrangelog)
	 	    {
	 	      gen[1] = &uri_wrangelog;	/* XXX fix me---somehow */
//
	 	      param.wrangelog.do_loop = (*optarg == 'y') || (*optarg == 'Y');
	 	      param.wrangelog.file = optarg + 2;
	             }
