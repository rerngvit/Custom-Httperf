...
static struct
  {
    ...
    latency_hist *latency_hist; /* histogram of latency */
    int num_latency_limits; // number of entries in the histogram of latency
    long num_latency_over_the_limits;  // number of requests over the last limit
...
static void count_latency_dist(Time cur_response_time)
{
	int j = 0;
	int low_limit = 0; // low limit start with 0 ms

	Time cur_response_time_ms = 1e3* cur_response_time; // convert to ms

	// count for the input range of latency
	for (j= 0; j < basic.num_latency_limits ; j++)
	{

		if ( cur_response_time_ms  > low_limit &&
				cur_response_time_ms <=  basic.latency_hist[j].latency_limit )
			basic.latency_hist[j].count++;
		low_limit = basic.latency_hist[j].latency_limit; // update low_limit to this current limit

	}

	// for the last range of latency
	if (cur_response_time_ms > basic.latency_hist[basic.num_latency_limits - 1].latency_limit)
		basic.num_latency_over_the_limits++;
}
...
recv_start (Event_Type et, Object *obj, Any_Type reg_arg, Any_Type call_arg)
{
  ...
  Time cur_response_time = now - c->basic.time_send_start;

  basic.call_response_sum += cur_response_time;

  if (param.wlatency_dist.file != NULL)
  count_latency_dist(cur_response_time);
...
void init_latency_hist()
{

	if (param.wlatency_dist.file == NULL || param.olatency_dist.file == NULL)
	{
		panic(" Error: both wlatency_dist and olatency_dist should be specified\n ");
	}

	FILE *file = fopen ( param.wlatency_dist.file, "r" );
	...
	if ( file != NULL )
	   {
		  ...
		  basic.latency_hist = (latency_hist*) malloc (line_count* sizeof(latency_hist));
		  basic.num_latency_limits = line_count;
		  basic.num_latency_over_the_limits = 0;
	      ...
	      while ( fgets ( line, sizeof line, file ) != NULL ) /* read a line */
	      {
	    	  ...
	    	  {
	    	     basic.latency_hist[index].latency_limit = number;
	    	     basic.latency_hist[index].count = 0;
	    	  }
	    	  index++;
	      }
	      fclose ( file );
	   ...
	   {
		  panic ("%s: can't open %s\n", prog_name,  param.wlatency_dist.file);
	   }
...
static void dump_latency_dist (void)
{
	if (param.wlatency_dist.show_latency)
	  {
		  ...
		  for (j=0; j < basic.num_latency_limits; j++)
		  {
			  printf("(%d,%d] ms => %ld requests\n", low_limit,
					  	  	  	  	  	  	  	  basic.latency_hist[j].latency_limit,
					  	  	  	  	  	  	  	  basic.latency_hist[j].count);
			  low_limit = basic.latency_hist[j].latency_limit;

		  }

		  int num_total_errors = basic.num_client_timeouts + basic.num_sock_timeouts
		  ...
		  printf("More than %d ms => %ld requests\n", basic.latency_hist[basic.num_latency_limits - 1].latency_limit,
				             basic.num_latency_over_the_limits + num_total_errors);

	  }

	if (param.olatency_dist.file != NULL)
		  {
			  FILE *fp;
			  fp=fopen(param.olatency_dist.file, "w+");
			   int j =0;
			  int low_limit = 0;
			  for (j=0; j < basic.num_latency_limits; j++)
			  {
			  ...
			  int latency_exceed_limit = basic.latency_hist[basic.num_latency_limits - 1].latency_limit + 1;
			  ...
			  fprintf(fp, "%d %ld\n", latency_exceed_limit ,
					  basic.num_latency_over_the_limits + num_total_errors);

			  fclose(fp);
		  }

	 free(basic.latency_hist);
}
