...
typedef struct Cmdline_Params
  {
    ...
    int num_det_bursts; /* number of det bursts */
    int num_conns;	/* # of connections to generate */
    int num_bursts; /* # of bursts attempt to try */
    ...
    struct
          {
    	char *file;	/* name of the file where range log entries are */
    	char do_loop;	/* boolean indicating if we want to loop on entries */
          }
    wrangelog;
    ...
    struct
    {
    	char show_latency;	/* boolean indicating if we want to show result in the summary page  */
    	char *file; /* name of the file where the latency limits are listed. */
    }
    wlatency_dist;

    struct
    {
     	char *file; /* name of the file where the latency distribution output would be printed */
    }
    olatency_dist;
